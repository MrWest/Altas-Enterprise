...
namespace CompanyName.Atlas.Investments.Tests.Implementation.Infrastructure.Data.Db4O.Budget
{
    ...
    public class PlannedResourceRepositoryBaseTests
    {
        ...
        public void GetItemCollection_ReturnsMethodAllowingToObtainCorrectPlannedResourceListOfCurrentBudgetComponent()
        {
            ...
            var budgetComponent = Mock.Of<IBudgetComponentItem>(x => Equals(x.PlannedResources , expectedList));
            var db = Mock.Of<IDb4ODatabaseContext>();
            var repository = new RepositoryStub(db);
            var internals = new PrivateObject(repository);

            // Act
            var method = (Func<IBudgetComponentItem, IList<IPlannedResource>>)internals.GetProperty("GetItemCollection");
        ...
        public class RepositoryStub : PlannedResourceRepositoryBase<IBudgetComponentItem>
        {
