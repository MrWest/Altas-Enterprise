...
namespace CompanyName.Atlas.Investments.Tests.UnitTests.Implementation.Domain.Services
{
    ...
    public class InvestmentElementDomainServicesTests : DomainServicesTestsBase<IInvestmentElement, InvestmentElementDomainServicesBase<IInvestmentElement>>
    {
        [TestInitialize]
        public override void Initialize()
        {
            ...
            ServiceLocatorMock.Setup(x => x.GetInstance<IInvestment>()).Returns(() =>
            {
                var mock = new Mock<IInvestment>();
                mock.SetupAllProperties();

                return mock.Object;
            });
            ServiceLocatorMock.Setup(x => x.GetInstance<IInvestmentComponent>()).Returns(() =>
            {
                var mock = new Mock<IInvestmentComponent>();
                mock.SetupAllProperties();
        ...
        [TestMethod]
        public void Create_ReturnsInvestmentInstance()
        {
            // Act
            IInvestmentElement element = TestObject.Create();

            // Assert
            Assert.IsInstanceOfType(element, typeof(IInvestmentElement));
        }
